...
func getCXLSpecs(cdiCache *cdiapi.Cache) []*cdiapi.Spec {
	cxlSpecs := []*cdiapi.Spec{}
	for _, cdiSpec := range cdiCache.GetVendorSpecs(device.CDIVendor) {
		if cdiSpec.Kind == device.CDIKind {
			cxlSpecs = append(cxlSpecs, cdiSpec)
		}
	}
	return cxlSpecs
}
...
func AddDetectedDevicesToCDIRegistry(cdiCache *cdiapi.Cache, detectedDevices device.DevicesInfo, doCleanup bool) error {
	cxlSpecs := getCXLSpecs(cdiCache)
	// delete all existing CXL specs.
	for _, spec := range cxlSpecs {
